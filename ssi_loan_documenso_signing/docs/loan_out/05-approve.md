...
>
> **Extends:** ssi_loan — model `loan.out`, aksi `05-approve`
...
- The behavior below only applies **when the record's active Approval Template has a
  ...
  `ssi_loan/docs/loan_out/05-approve.md`.
...
- Instead of clicking Approve, the user opens the linked request — shown under the
  ...
  **Signature Requests** tab (or via the **Open Signature Requests** button in the same
  ...
  Odoo, in Documenso.
- Approval completes automatically — without any button click on this Loan Out record —
  ...
  **Check Status** on the request, or synced automatically by the connector). At that
  point, the base Post-Condition applies as usual: the loan realization journal entry is
  created and the status changes to **Ready to Process** (or remains **Waiting for
  Approval** if other approval levels are still pending).
